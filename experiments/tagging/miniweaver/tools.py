...
def _repeat_pad(a, maxlen, dtype="float64"):
...
def _fused_pad_and_stack(table, var_names, preprocess_params, dtype="float64"):
    ...
    content_arrays = []
    param_centers = np.empty(n_vars, dtype=np.float64)
    param_scales = np.empty(n_vars, dtype=np.float64)
    param_los = np.empty(n_vars, dtype=np.float64)
    param_his = np.empty(n_vars, dtype=np.float64)
    param_do_centers = np.empty(n_vars, dtype=np.bool_)
    param_pad_values = np.empty(n_vars, dtype=np.float64)
    ...
    for vi, vn in enumerate(var_names):
        ...
        param_centers[vi] = np.float64(p["center"]) if dc else np.float64(0.0)
        param_scales[vi] = np.float64(p["scale"])
        # None min/max means no clipping — use ±inf so comparisons never trigger
        param_los[vi] = np.float64(p["min"]) if p["min"] is not None else np.float64(-np.inf)
        param_his[vi] = np.float64(p["max"]) if p["max"] is not None else np.float64(np.inf)
        param_pad_values[vi] = np.float64(p.get("pad_value", 0))
        content_f64 = content.astype(np.float64) if content.dtype != np.float64 else content
        content_arrays.append(content_f64)

    content_len = int(shared_offsets[-1])
    all_content = np.empty(n_vars * content_len, dtype=np.float64)
    ...
    if pad_mode == "wrap":
        _batched_fused_repeat_pad(
            all_content, content_starts, shared_offsets,
            param_centers, param_scales, param_los, param_his,
            param_do_centers, out,
        )
    else:
        _batched_fused_constant_pad(
            all_content, content_starts, shared_offsets,
            param_centers, param_scales, param_los, param_his,
